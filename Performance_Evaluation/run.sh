...
# ==============================================================================
# 联邦学习安全聚合方案性能自动化评估脚本 (HPC & 强鲁棒性优化版)
# ==============================================================================
    ...
    for c in "${CLIENT_COUNTS[@]}"; do
        ...
        for p_pow in "${PARAM_POWERS[@]}"; do
            ...
            if [ "$c" -ge 10000 ] && [ "$p_pow" -ge 20 ]; then
                ...
                WAIT_TIME=60
                TIMEOUT_VAL=3600   # 1小时
            elif [ "$c" -ge 1000 ]; then
                WAIT_TIME=30
                TIMEOUT_VAL=3600
            elif [ "$c" -ge 100 ]; then
                WAIT_TIME=10
                TIMEOUT_VAL=1800   # 30分钟
            else
                WAIT_TIME=3
                TIMEOUT_VAL=600    # 10分钟
            fi
            ...
            echo "" >> "$LOG_FILE"
            echo "--------------------------------------------------------" >> "$LOG_FILE"
            echo " 测试配置 -> Params: 2^$p_pow ($param_size) | Port: $CURRENT_PORT | Timeout: ${TIMEOUT_VAL}s " >> "$LOG_FILE"
            echo "--------------------------------------------------------" >> "$LOG_FILE"
            ...
            if [ $EXIT_CODE -eq 127 ]; then
                ...
                # 目录都不存在，无需 sleep 和消耗端口
                continue 
            elif [ $EXIT_CODE -eq 124 ]; then
                ERR_MSG="$MARK_TIMEOUT [TIMEOUT] 方案 $scheme | 客户端 $c | 参数 2^$p_pow | 端口 $CURRENT_PORT | 超过 ${TIMEOUT_VAL}s 被强杀"
                echo -e "  $ERR_MSG"
                echo "$ERR_MSG" >> "$FAILED_LOG"
                echo "$ERR_MSG" >> "$LOG_FILE"
