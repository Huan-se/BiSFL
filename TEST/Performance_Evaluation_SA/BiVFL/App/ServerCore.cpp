...
// [修改] Server 在线聚合，直接使用 TA 传来的预计算数组 ta_s_alpha 和 ta_s_beta
void aggregate_and_unmask_sparse(
    int* u1_ids, int u1_len, int* u2_ids, int u2_len, 
    int64_t* shares_flat, int shares_len, int64_t* ciphers_flat, int param_size, 
    const char* kappa_m_str, int t, const char* model_hash_str, int threshold, 
    const long long* ta_s_alpha, const long long* ta_s_beta, // <-- TA 离线数组
    float* output_result 
) {
    long kappa_m = std::strtol(kappa_m_str, NULL, 10);
    ...
    for (int k = 0; k < param_size; ++k) {
        ...
        for(int i=0; i<u2_len; ++i) sum_cipher = MathUtils::safe_mod_add(sum_cipher, ciphers_flat[i * param_size + k]);
