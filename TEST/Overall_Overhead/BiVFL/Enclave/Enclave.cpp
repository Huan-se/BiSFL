...
int printf(const char *fmt, ...) {
    ...
    vsnprintf(buf, BUFSIZ, fmt, ap); va_end(ap);
    ocall_print_string(buf); return 0;
}
...
public:
    FastAESRandom(long seed) {
        memset(&key, 0, sizeof(key)); memset(ctr, 0, sizeof(ctr));
    ...
    void generate_rademacher_chunk(Eigen::VectorXf& chunk, int size) {
        ...
        for (int i = 0; i < size; ++i) chunk[i] = (rands[i] & 1) ? 1.0f : -1.0f;
    }
};
...
void GenerateGraph(int n, long kappa_m, int t, std::vector<int>& u1_ids, int my_id, 
    ...
    int my_idx = -1;
    for(int i=0; i<n; ++i) { if(u1_ids[i] == my_id) { my_idx = i; break; } }
    if(my_idx == -1) return;
    int my_logical = -1;
    for(int i=0; i<n; ++i) { if(pi[i] == my_idx) { my_logical = i; break; } }
    for(int d = 1; d <= K / 2; ++d) {
        out_neighbors.push_back(u1_ids[pi[(my_logical + d) % n]]);
        out_neighbors.push_back(u1_ids[pi[(my_logical - d + n) % n]]);
    }
}
...
void ecall_prepare_gradient(
    int client_id, const char* proj_seed_str, float* w_new, float* w_old, size_t model_len, 
    ...
    try {
        ...
        for(size_t i = 0; i < model_len; ++i) full_gradient.push_back(w_new[i] - w_old[i]);
        ...
        for (size_t k = 0; k < out_len; ++k) {
            ...
            for (size_t r = 0; r < ranges_len; r += 2) {
                ...
                while (offset < block_len) {
                    ...
                    Eigen::Map<Eigen::VectorXf> grad_segment(full_gradient.data() + start_idx + offset, curr_size);
...
void ecall_get_vector_shares_dynamic(
    ...
    for(int a : alive_neighbors) generate_share(1, a, CryptoUtils::derive_seed(kappa_m, "self", a * 1000 + t));
