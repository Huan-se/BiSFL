...
class Client(object):
    ...
    def tee_step1_encrypt(self, w, active_ids, kappa_m, t, model_hash_str):
        ...
        c_grad = self.tee_adapter.generate_masked_gradient_dynamic(kappa_m, t, model_hash_str, self.client_id, active_ids, w, model_len)
        return c_grad

    def tee_step2_generate_shares(self, kappa_s, kappa_m, t, u1_ids, u2_ids):
        flat_shares = self.tee_adapter.get_vector_shares_dynamic(kappa_s, kappa_m, t, u1_ids, u2_ids, self.client_id)
        count = int(flat_shares[0])
        structured_shares = []
        idx = 1
        for _ in range(count):
            if idx + 2 >= len(flat_shares): break
            structured_shares.append((int(flat_shares[idx]), int(flat_shares[idx+1]), int(flat_shares[idx+2])))
            idx += 3
