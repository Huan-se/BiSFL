...
class ServerSimulator:
    def __init__(self, host='0.0.0.0', port=8888, num_clients=20, param_size=1000000, drop_rate=0.1):
        ...
        self.client_sockets = {}
        self.public_keys = {}  # {uid: (c_pk, s_pk)}
        
        print("[Server] 初始化 Server 端环境 (Double-Masking Version)...")
    ...
    def wait_for_clients(self):
        ...
        while connected_count < self.num_clients:
            ...
            client_id = connected_count + 1  # 为了配合 Shamir 分片，ID 从 1 开始
    ...
    def run_simulation_round(self):
        ...
        req_msg_r1 = {
            "action": "REQ_ROUND1", 
            "public_keys_dict": self.public_keys,
            "threshold": threshold
        }
        ...
        # 路由重组: 解析每个客户端发来的加密分片
        # r1_results[sender]["ciphertexts"][receiver] = ciphertext
        ...
        # 模拟部分客户端在 Round 2 到 Round 4 之间掉线
        ...
        b_u_shares = {u: {} for u in u3_ids}         # 存活者的 b_u 分片
        s_sk_shares = {u: {} for u in dropped_ids}   # 掉线者的 s_sk 分片
        
        for sender_id, res in r4_results.items():
            self.comm_bytes_round4 += self._get_msg_size(res)
            # share_dict 结构: {target_id: {"type": "b_u" / "s_sk", "share": value}}
            for target_id, share_info in res["shares"].items():
                x_coord = sender_id + 1  # 对应 client 生成分片时使用的索引
        ...
        for u in dropped_ids:
            if len(s_sk_shares[u]) >= threshold:
                ...
                for v in u3_ids:
                    # 只有 v 存活，其上传的密文中才包含和 u 相关的掩码，需要将其抵消
                    s_uv = CryptoUtils.agree(s_sk_bytes, self.public_keys[v][1])
                    p_uv = CryptoUtils.generate_mask(s_uv, self.param_size, MOD)
                    
                    if u > v:
                        # 客户端 u (掉线) 曾 +p_uv，v (存活) 曾 -p_uv。
                        # 因为 u 未上传，总和中多出了 -p_uv，所以 Server 需要 +p_uv 抵消
                        Z = (Z + p_uv) % MOD
                    elif u < v:
                        # u (掉线) 曾 -p_uv，v (存活) 曾 +p_uv。Server 需要 -p_uv 抵消
                        Z = (Z - p_uv) % MOD
        ...
        t_r4 = (time.time() - t_start_r4) - t_agg  # 减去计算时间，得到纯通信时间

        # ==========================================================
        # 通信量与时间报告 (与 TEE Baseline 格式对齐)
        # ==========================================
        ...
        print(f" 5. Server 聚合与解密计算耗时 : {t_agg:.4f} s (纯 Server 端高强度计算)")
        print(" =================================================================")
