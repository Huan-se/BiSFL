...
void tee_set_verbose(int level) { if (global_eid != 0) ecall_set_verbose(global_eid, level); }
...
void tee_prepare_gradient(
    ...
    sgx_status_t ret = ecall_prepare_gradient(global_eid, client_id, proj_seed_str, w_new, w_old, model_len, ranges, ranges_len, output_proj, out_len);
...
void tee_generate_masked_gradient_dynamic(
    ...
    int client_id, int* u1_ids, size_t u1_len, const char* k_weight_str, 
    size_t model_len, int* ranges, size_t ranges_len, long long* output, size_t out_len
) {
    sgx_status_t ret = ecall_generate_masked_gradient_dynamic(global_eid, kappa_m_str, t, model_hash_str, client_id, u1_ids, u1_len, k_weight_str, model_len, ranges, ranges_len, output, out_len);
    if (ret != SGX_SUCCESS) print_error_message(ret);
}
...
void tee_get_vector_shares_dynamic(
    ...
    sgx_status_t ret = ecall_get_vector_shares_dynamic(global_eid, kappa_s_str, kappa_m_str, t, u1_ids, u1_len, u2_ids, u2_len, my_id, output_vector, max_len);
    if (ret != SGX_SUCCESS) print_error_message(ret);
}

void tee_generate_noise_from_seed(const char* seed_str, size_t len, long long* output) {
    sgx_status_t ret = ecall_generate_noise_from_seed(global_eid, seed_str, len, output);
    if (ret != SGX_SUCCESS) print_error_message(ret);
}
