...
class Server(object):
    ...
    def calculate_weights(self, client_id_list, client_features_dict_list, client_data_sizes, current_round=0, client_objects=None):
        ...
        if any(k in self.detection_method for k in ["mesas", "projected", "layers_proj", "ours"]):
            ...
            if self.verbose: print(f"  [Server] Executing Baseline {self.detection_method.upper()} detection (Round {current_round})...")
            ...
            if self.log_file_path: self._write_detection_log(current_round, logs, weights, global_stats)
        else:
                ...
                for cid in sorted(logs.keys()):
                    ...
                    for scope in scopes:
                        for metric in metrics_list:
                            ...
                            row.extend([f"{val:.4f}", f"{median:.4f}", f"{thresh:.4f}"])
                    writer.writerow(row)
        except Exception: pass

    def secure_aggregation(self, client_objects, active_ids, round_num):
        t_start_total = time.time()
        if self.verbose: print(f"\n[Server] >>> STARTING SECURE AGGREGATION (ROUND {round_num}) [View-Binding SSS] <<<")
        ...
        for cid in u2_ids:
            ...
            try:
                shares = client.tee_step2_generate_shares(
                    self.kappa_s, self.kappa_m, round_num, sorted_active_ids, u2_ids
                )
        ...
        if len(shares_list) != len(u2_ids):
            print(f"  [Abort] Consistency Broken! U2 size={len(u2_ids)}, Shares={len(shares_list)}.")
            return
        t_s2_end = time.time()

        # Step 3: ServerCore 聚合
        t_agg_start = time.time()
        if self.verbose: print("  [Step 3] Executing ServerCore Aggregation...")
        try:
            # [修复重点]：将 TA 的累加合并在 C++ 内部流式处理
            result_int = self.server_adapter.aggregate_and_unmask(
                sorted_active_ids, u2_ids, shares_list, final_ciphers, 
                self.kappa_m, round_num, model_hash_str, threshold
            )
            ...
            if self.verbose: print("  [Success] Aggregation Completed.")
        except Exception as e:
            print(f"  [Critical Error] Aggregation crashed: {e}")

        t_agg_end = time.time()
        if self.verbose:
            print(f"  [Perf] Time Breakdown: S1: {t_s1_end-t_s1_start:.4f}s | S2: {t_s2_end-t_s2_start:.4f}s | Core: {t_agg_end-t_agg_start:.4f}s")
