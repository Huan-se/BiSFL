...
long long lagrange_interpolate_zero(const std::vector<int>& xs, const std::vector<long long>& ys) {
    ...
    for (int i = 0; i < k; ++i) {
        ...
        for (int j = 0; j < k; ++j) {
            if (i != j) {
                num = MathUtils::safe_mod_mul(num, -xs[j]);
...
void server_core_aggregate_and_unmask(
    ...
    long long* shares_flat, int shares_len, 
    long long* ciphers_flat, int data_len, 
    const char* kappa_m_str, int t,
    const char* model_hash_str, int threshold,
    long long* output_result
) {
    long kappa_m = std::strtol(kappa_m_str, NULL, 10);
    long long H_val = std::stoll(model_hash_str) % MOD;

    // Parse shares: Map TargetID -> vector of (x, y)
    std::map<int, std::vector<std::pair<int, long long>>> shares_self;
    std::map<int, std::vector<std::pair<int, long long>>> shares_alpha;
    std::map<int, std::vector<std::pair<int, long long>>> shares_beta;

    for (int i = 0; i < shares_len; i += 4) {
        int source = shares_flat[i];
        int tag    = shares_flat[i+1];
        int target = shares_flat[i+2];
        long long val = shares_flat[i+3];
    ...
    auto interpolate = [&](const std::vector<std::pair<int, long long>>& pts) -> long long {
    ...
    // Recover seeds
    std::vector<DeterministicRandom> active_self_rngs;
    std::vector<DeterministicRandom> dropped_alpha_rngs;
    std::vector<DeterministicRandom> dropped_beta_rngs;

    for(auto& kv : shares_self) {
        long long s = interpolate(kv.second);
        if(s > 0) active_self_rngs.emplace_back((long)s);
    }
    for(auto& kv : shares_alpha) {
        long long s = interpolate(kv.second);
        if(s > 0) dropped_alpha_rngs.emplace_back((long)s);
    }
    for(auto& kv : shares_beta) {
        long long s = interpolate(kv.second);
        if(s > 0) dropped_beta_rngs.emplace_back((long)s);
    }

    // [修改重点]：为 TA 生成 U1 的流式生成器池
    std::vector<DeterministicRandom> ta_alpha_rngs;
    std::vector<DeterministicRandom> ta_beta_rngs;
    for (int i = 0; i < u1_len; ++i) {
        long s_a = CryptoUtils::derive_seed(kappa_m, "alpha", u1_ids[i] * 1000 + t);
        long s_b = CryptoUtils::derive_seed(kappa_m, "beta",  u1_ids[i] * 1000 + t);
        ta_alpha_rngs.emplace_back(s_a);
        ta_beta_rngs.emplace_back(s_b);
    }

    // Streaming Aggregation and Mask Cancellation
    for (int k = 0; k < data_len; ++k) {
        long long sum_cipher = 0;
        for(int i=0; i<u2_len; ++i) {
            sum_cipher = MathUtils::safe_mod_add(sum_cipher, ciphers_flat[i * data_len + k]);
        }

        // 动态推进 TA 的累加流
        long long cur_S_alpha = 0;
        long long cur_S_beta  = 0;
        ...
        output_result[k] = MathUtils::safe_mod_sub(sum_cipher, noise);
    }
}
}